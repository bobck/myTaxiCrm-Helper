...
exports.up = async function (db, cb) {
  ...
  const seedData = mapping.flatMap((park) => {
    return park.rule_ids.map((ruleId, index) => ({
      id: ruleId,
      auto_park_id: park.auto_park_id,
      weekDay: index + 2, //index (0) + 2 = 2 (Tuesday)
    }));
  });
  const insertPromises = seedData.map((data) => {
    const sql = `
      INSERT INTO catalog_tariffs (id, auto_park_id, "weekDay") 
      VALUES ('${data.id}', '${data.auto_park_id}', ${data.weekDay});
    `;
    const values = [data.id, data.auto_park_id, data.weekDay];
    return db.runSql(sql, cb);
  });
