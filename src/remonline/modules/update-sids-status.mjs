...
export async function checkIsSidStatusWasUpdated() {
  ...
  const result = await getSidsNotReadyDoBeClosed({
    statusesToBeClosed: statusesToBeClosed.join(','),
  });
  ...
  console.log({
    time: new Date(),
    message: 'checkIsSidStatusWasUpdated',
    sidsNotReadyDoBeClosed: result.length,
  ...
  for (let order of orders) {
    ...
    if (sameSid.status_id != order.status.id) {
      console.log(
        `Status was updated ${order.id}|${sameSid.sid_id} - ${order.id_label} - ${sameSid.sid_id}`
      );
      let isClosed = false;
