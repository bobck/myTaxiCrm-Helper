...
export async function saveUniqWorkedDriversAndAvgLifeTime(dateTime) {
  ...
  console.log({
    ...
    week,
    year,
    message: 'saveUniqWorkedDriversAndAvgLifeTime',
  });
  ...
  let firstDayOfTargetYear = DateTime.fromObject({
    year: year,
    month: 1,
    day: 1,
  });
  if (firstDayOfTargetYear.weekday !== 1) {
    firstDayOfTargetYear = firstDayOfTargetYear.plus({
      days: 8 - firstDayOfTargetYear.weekday,
    });
  }
  ...
  const jsonData = rows.map((row) => {
    ...
    const dateFromSQL = DateTime.fromSQL(driver_created_at, {
      zone: 'europe/kyiv',
    });
    ...
    if (year != currentYear || week != currentWeek) {
      const difference = lastDayOfTargetWeek
        .diff(dateFromSQL, 'days')
        .as('days');
      life_days = Math.abs(difference);
    }
  ...
  await loadJsonToTable({
    json: tempFilePath,
    bqTableId,
    schema: uniqWorkedDriversAndAvgLifeTimeTableSchema,
  });
