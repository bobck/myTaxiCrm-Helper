...
export async function saveCarUsageReport(manualDate) {
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_car_usage_report.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: carUsageReportTableSchema });
