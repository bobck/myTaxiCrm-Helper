...
export async function saveMileagesAndHoursOnline(dateTime) {
    ...
    console.log({ time: new Date(), week, year, message: 'saveMileagesAndHoursOnline' });
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_mileages_and_hours_online.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: mileagesAndHoursOnlineTableSchema });
