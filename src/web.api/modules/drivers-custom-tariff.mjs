...
export async function setDriversCustomTariff() {
  ...
  for (const driver of newDrivers) {
    ...
    const { rule_ids } = mapping.find(
      (rule) => rule.auto_park_id == auto_park_id
    );

    const hireDate = new DateTime(created_at);
    const ruleIndex = hireDate.weekday - 2; // because first rule is for tuesday the last one for friday
