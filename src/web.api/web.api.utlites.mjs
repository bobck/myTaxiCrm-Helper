...
export async function getDiscountTariffsForAutoparksByDay({ dayOfWeek, companyId, autoParksIds }) {
    ...
    for (let autoParkId of uniqueAutoParkIds) {
        ...
        const { name, tax_percent, tax_type, target_marker, accounting, divisible_income_type, driver_fleet_bonuses_percent } = firstRow;
...
export async function getDriverTripsCountByPeriod({ driver_id, auto_park_id, periodStartDate, periodEndDate }) {
    const sql = fs.readFileSync("src/sql/get_driver_trips_count_by_period.sql").toString();
    const result = await pool.query(sql, [driver_id, auto_park_id, periodStartDate, periodEndDate]);
    const { rows, rowCount } = result;
