...
export async function saveActiveDriversWithScheduleEvents(manualDate) {
    ...
    console.log({ time: new Date(), date, message: 'saveActiveDriversWithScheduleEvents' });
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_actives_with_schedule_events.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: activeDriversWithScheduleEventsTableSchema });
