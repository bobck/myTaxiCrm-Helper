...
export async function saveActiveDriversWithScheduleCompany(manualDate) {
    ...
    console.log({ time: new Date(), date, message: 'saveActiveDriversWithScheduleCompany' });
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_actives_with_schedule_company.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: activeDriversWithScheduleCompanyTableSchema });
