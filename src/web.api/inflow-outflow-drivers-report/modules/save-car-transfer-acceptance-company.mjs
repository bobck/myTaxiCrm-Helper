...
export async function saveCarTransferAcceptanceCompany(manualDate) {
    ...
    console.log({ time: new Date(), date, message: 'saveCarTransferAcceptanceCompany' });

    const { rows } = await getCarTransferAcceptanceCompany({ date });

    console.log({ getCarTransferAcceptanceCompany: rows.length })
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_car_transfer_acceptance_company.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: carTransferAcceptanceCompanyTableSchema });
