...
export async function getAllReferrals({
  ...
  let allItems = [];
  let start = 0;
  const BATCH_SIZE = 50; // Bitrix API default limit per request for crm.item.list

  while (true) {
    const params = {
      entityTypeId: referralTypeId,
      order: { ID: 'ASC' }, // Consistent ordering is good for pagination
      select: selectFields,
      start: start,
    };
    ...
    if (
      ...
      console.error(
        'getAllReferrals: Unexpected response structure from Bitrix.',
        { referralTypeId, start }
      );
      // Log the actual response for debugging if possible, but be mindful of sensitive data
      // console.error('Problematic Bitrix response:', response);
      throw new Error(
        `Failed to fetch referrals for entityTypeId ${referralTypeId} due to unexpected API response.`
      );
    }
    ...
    // As per your requirement: stop downloading when the amount of returned items is lower than 50
    if (fetchedItems.length < BATCH_SIZE) {
      break;
    }

    // If we fetched a full batch (BATCH_SIZE items),
    // we need the 'next' value to continue.
    // The 'next' property indicates the starting position for the next set of items.
    if (response.next && Number(response.next) > start) {
      start = Number(response.next);
    } else {
      // Fetched a full batch, but no 'next' pointer,
      // or 'next' is not progressive. Assume end of data.
      break;
    }
  }
  return allItems;
