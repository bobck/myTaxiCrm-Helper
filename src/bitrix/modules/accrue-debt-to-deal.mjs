...
export async function accrueDebtToDeal() {
  ...
  for (const human_id of human_ids) {
    ...
    if (
      purpose == 'VOLUNTARY_PAY_OFF_DEBT' ||
      purpose == 'PAY_OFF_DEBT_DEPOSIT_ACCOUNT'
    ) {
      const ufCrmField = 'UF_CRM_1654075693';
      await voluntaryPayOffToDeal({
        voluntaryPayOff,
        sum,
        dtp_deal_id,
        added_by_user_name,
...
async function forcedPayOffToDeal({
  forcedPayOff,
  sum,
  dtp_deal_id,
  added_by_user_name,
  ...
  const { result } = await updateDealPayOff({
    id: dtp_deal_id,
    ufCrmField,
    amount: currentForcedPayOff,
  });
...
async function accruedDebtToDeal({
  ...
  const { result } = await updateDealDebt({
    id: dtp_deal_id,
    debt: currentDebt,
  });
