...
function getRefferalsForPayProcentageReward({ activeRefferals, revenueDetails, week, year }) {
    ...
    for (let referral of activeRefferals) {
        ...
        const revenueDetail = revenueDetails.find(revenue => revenue.driver_id === driver_id);
        ...
        if (createdAtWeek == week && createdAtYear == year) {
            ...
            if (revenueDetail && revenueDetail.driver_revenue >= targetMarker) {
                const rewardAmount = Math.round(revenueDetail.driver_revenue * rewardProcentage)
                refferalsReadyForPay.push({ ...referral, ...revenueDetail, targetMarker, rewardAmount });
                continue;
            }
...
export async function createRefferalPaymentProcentageReward() {
    ...
    const { activeRefferals } = await getActiveRefferalsProcentageReward({ date, procentageRewardAutoParkIds });
    ...
    const { rows: revenueDetails } = await getRevenueDetailsForRefferalsProcentageReward({ activeRefferalsIds, week, year });
    const { refferalsReadyForPay, refferalsNotEligible } = getRefferalsForPayProcentageReward({ activeRefferals, revenueDetails, week, year })
    ...
    for (let refferal of refferalsReadyForPay) {
        ...
        console.log({ referral: 'refferalsReadyForPay', referral_id, week, year, created_date });
    ...
    for (let refferal of refferalsNotEligible) {
        ...
        console.log({ referral: 'refferalsNotEligible', referral_id, created_date, week, year });
