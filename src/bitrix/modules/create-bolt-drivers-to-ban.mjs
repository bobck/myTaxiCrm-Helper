...
import { getBoltDriverBanReqByDriverId, insertBoltDriverBanReq } from "../bitrix.queries.mjs";
...
export const createBoltDriversToBan = async () => {
    ...
    for (const [index,row] of rows.entries()) {
        ...
        if (dbcard) {
            console.error(`Present driver bolt driver ban request card while creating driver_id:${driver_id}`);
            continue;
        }
