...
export async function syncRevenueToDeals() {
    ...
    const { driversWithRevenueWitnDeal } = await getDriversWithRevenueWitnDealSyncReady();


    const driversWithRevenueWitnDealChunked = chunkArray(driversWithRevenueWitnDeal, 50);
    ...
    for (let chunk of driversWithRevenueWitnDealChunked) {
        ...
        try {
            const { result, result_time } = await updateDealsOpportunity({ drivers: chunk })
    ...
    console.log(`chunkArray done in: ${Math.round((endTotal - startTotal) / 1000)} sec.`);
