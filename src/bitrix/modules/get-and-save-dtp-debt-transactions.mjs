...
function formatCreatedAt(createdAt) {
  return DateTime.fromJSDate(createdAt, { zone: 'UTC' }).toSQL();
}
...
export async function getAndSaveDtpDebtTransactions() {
  ...
  console.log({
    time: new Date(),
    message: 'getAndSaveDtpDebtTransactions',
    created_at,
  });

  const { rows } = await getDtpDebtTransactions({
    createdAt: DateTime.fromSQL(created_at).plus({ milliseconds: 1 }).toSQL(),
  });
  ...
  const dtpDebtTransactions = rows.map((row) => {
    ...
    return {
      ...
      added_by_user_name,
      dtp_deal_id,
      created_at: formatCreatedAt(created_at),
    };
  });
