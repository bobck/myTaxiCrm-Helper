...
export async function contactsForDriversWithRevenue() {
    ...
    const { result } = await findContactsByPhones({ drivers: driversWithRevenue });

    const driversWithRevenueAndContacts = driversWithRevenue.map((element, index) => {
        return {
            ...element,
...
export async function dealForDriversWithRevenue({ category_id, companyIds }) {
    const { driversWithRevenue } = await getAllDriversWithRevenueWitnConcactsAndNoDeals({ companyIds })
    const { result } = await findDealByContact({ drivers: driversWithRevenue, category_id })
...
if (process.env.ENV == "TEST") {
    ...
    await dealForDriversWithRevenue({ category_id: 3, companyIds: ownCompanyIds });

    const b2cCompanyIds = ['3b63b0cc-155b-43c5-a58a-979f6aac0d35']
    await dealForDriversWithRevenue({ category_id: 0, companyIds: b2cCompanyIds });
}
