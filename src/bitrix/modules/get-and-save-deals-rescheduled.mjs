...
export async function getAndSaveDealsRescheduled(dateTime) {
    ...
    console.log({ time: new Date(), week, year, message: 'getAndSaveDealsRescheduled' });
    ...
    const tempFilePath = path.join(os.tmpdir(), 'temp_data_deals_hr_rescheduled.json');
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: dealsHrRescheduledTableSchema });
