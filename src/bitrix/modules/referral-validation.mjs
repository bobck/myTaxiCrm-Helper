import { deleteBitrixTaskById, addCommentToDeal } from '../bitrix.utils.mjs';
...
export async function referralValidadion(params) {
  ...
  if (
    ...
    await addCommentToDeal({
      deal_id,
      comment: 'Відмовлено у рефелальній програмі. Недостатньо данних',
    });
    return false;
  }

  const { rows } = await getDriverByContract({
