...
export async function insertBrandingCard(card) {
    const { driver_id, crm_card_id, total_trips, weekNumber, year } = card;
    ...
    const sql = `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    return db.run(sql, driver_id, crm_card_id, total_trips, weekNumber, year, created_at, updated_at);
}
 ...
 */
export async function updateBrandingCardsByDriverId({ driver_id, weekNumber, total_trips }) {
    const updated_at = new Date().toISOString();
...
export const testBrandingCards = async () => {
    try {
        ...
        const changes = await updateBrandingCardsByDriverId({
            driver_id: sampleCard.driver_id,
            total_trips: "20",
            weekNumber: sampleCard.weekNumber,
        });
...
export async function updateManifoldDealsAncidentData(manifoldDealsData) {
    ...
    try {
        for (let [id, data] of Object.entries(manifoldDealsData)) {
            const { UF_CRM_1654602086875: accident_id, UF_CRM_1672920789484: aviable_for_office_only } = data;
...
export async function saveReferralIdForRecruitDeal({ deal_id, referral_id, task_id }) {
    const sql = `UPDATE referral SET referral_id = ? WHERE deal_id = ? AND task_id = ?`;

    await db.run(sql, referral_id, deal_id, task_id);
}

export async function approvalReferralById({ referral_id, referrer_phone, referrer_name, referrer_position }) {
    const sql = `UPDATE 
                    referral 
                SET is_approved = TRUE,
                    ...
                    referrer_position = ?
                WHERE referral_id = ?`;
    await db.run(sql, referrer_phone, referrer_name, referrer_position, referral_id);
}
...
export async function insertNewWorkingDriver({ id, auto_park_id, first_name, last_name, phone, created_at, item_id }) {
    await db.run(
