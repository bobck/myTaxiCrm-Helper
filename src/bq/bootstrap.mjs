...
import { resetAndLoadOrderStatusesJob } from './jobs/reset-and-load-order-statuses-job.mjs';
export function bqJobs() {
  ...
  try {
    ...
    resetAndLoadOrderStatusesJob.start();
  } catch (error) {
    ...
    resetAndLoadOrderStatusesJob.stop();
    bqJobs();
  }
}
