...
export const loadRemonlineOrderProductPricesToBQ = async (ids) => {
  ...
  for (const [i, { order_id }] of order_ids.entries()) {
    ...
    const check = (product) =>
      product.prices['389671'] != product.prices['389671'];
    const isFound = parsedProducts.some(check);
    devLog(`${i} interation done, found:${isFound}, processed:${processed}`);
    processed++;
    if (isFound) {
      devLog(order_id, parsedProducts.find(check));
      break;
    }
  }
};
