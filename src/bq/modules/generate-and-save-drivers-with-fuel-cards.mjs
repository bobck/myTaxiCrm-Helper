...
export async function generateAndSaveDriversWithFuelCardsReport() {
    ...
    await insertRowsAsStream({ rows: rowsMapped, bqTableId: 'drivers_with_fuel_card_report' });
}
...
if (process.env.ENV == "REWRITE_REPORT") {
    ...
    await insertRowsAsStream({ rows: mapRows, bqTableId: 'drivers_with_fuel_card_report' });
