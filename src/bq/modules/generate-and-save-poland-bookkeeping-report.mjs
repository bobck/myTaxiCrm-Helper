...
const modelPrices = [
    {
        autoParkId: 'de4bf8ba-30c2-452c-a688-104063052961', autoParkName: 'Warsaw', modelPrices: [
    ...
    {
        autoParkId: 'be6ab23a-d6ba-4add-b0f7-cfb8abd0586b', autoParkName: 'Kraków', modelPrices: [
...
export async function generateAndSavePolandBookkeepingReport({ autoParkId }) {
    ...
    console.log({ time: new Date(), periodFrom, periodTo, year, week, autoParkId, message: 'generateAndSavePolandBookkeepingReport' });

    await clearTableByWeekAndYearAndAutoParkId({ bqTableId, year, week, autoParkId });
    const { rows } = await generatePolandBookkeepingReport({ periodFrom, periodTo, autoParkId });
    ...
    const jsonData = rows.map(row => {
        ...
        const [autoParkModelPrices] = modelPrices.filter(a => a.autoParkId == autoParkId)

        const [priceRow] = autoParkModelPrices.modelPrices.filter(m => m.modelId == model_id)
    ...
    const tempFilePath = path.join(os.tmpdir(), `temp_data_poland_bookkeeping_report_${autoParkId}.json`);
    const jsonString = jsonData.map(JSON.stringify).join('\n');
    fs.writeFileSync(tempFilePath, jsonString);

    await loadJsonToTable({ json: tempFilePath, bqTableId, schema: polandBookkeepingReportTableSchema });
