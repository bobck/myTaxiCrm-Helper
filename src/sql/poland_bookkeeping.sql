WITH
  drivers_with_income AS (
    SELECT
      ...
      drc.auto_park_id,
      MIN(drc.date AT TIME zone 'europe/warsaw') AS period_from,
      MAX(drc.date AT TIME zone 'europe/warsaw') AS period_to
    FROM
    ...
    WHERE
      ...
      AND drc.date AT TIME zone 'europe/warsaw' >= $1
      AND drc.date AT TIME zone 'europe/warsaw' <= $2
    GROUP BY
      drc.driver_id,
      ap.name,
      drc.auto_park_id
